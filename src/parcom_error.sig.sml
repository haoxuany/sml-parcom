...
signature PARCOM_ERROR = sig
  ...
  val map : ('a -> 'b) -> 'a t -> 'b t
  val mapError : (error -> error) -> 'a t -> 'a t
  val bind : 'a t -> ('a -> 'b t) -> 'b t
  val return : 'a -> 'a t
  val returnError : error -> 'a t

  datatype 'a parse_result =
    ParseSuccess of 'a
  | ParseFailure of error

  val terminal : (token -> 'a parse_result) -> 'a t
  ...
  * because of memoization in Johnson. *)
  val terminals :
    (stream ->  ('a * int * stream) parse_result) -> 'a t
  ...
  * memoize yourself. *)
  ...
  datatype 'a result =
    ResultSuccess of ( 'a * stream ) list
  | ResultFailure of ( error * stream ) list

  val parser : 'a t_memo -> stream -> 'a result
end
